...
class NFCustomerOrders extends HTMLElement {
  ...
  fetchProductOrderList = async () => {
    ...
    try {
      ...
      productListData?.data.forEach(product => {
        ...
        const piecesPerBoxMetafield = this.findMetafield(productMetafields, 'woo', 'short_description_plain');
        if (piecesPerBoxMetafield) {
          ...
          if (productElement) {
            ...
            productElement.innerHTML = `<span>${window.boxIcon}</span>${piecesPerBoxMetafield.value} `;
          }
        }
      });
      }
    ...
    if (orderListData.length > 0) {
      ...
      fetch('/cart/add.js', {
        ...
        .then(json => {
          ...
          this.updateCartButtonUI(window.String.reOrderSelectedItems);
        })
        .catch(error => {
  ...
  createDraftOrder() {
    ...
    createDraftOrderButton.innerHTML = `<div class="nf-loader-cta">
            <div class="nf-spinner"></div>
          </div>`;
  ...
  updateCartButtonUI(message) {
    ...
    if (cartButton) {
      ...
      setTimeout(() => {
        cartButton.innerHTML = `${window.String.reOrderSelectedItems}`;
      }, 3000);
    }
  }
